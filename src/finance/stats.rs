...
/// Calcula los retornos diarios de una serie de precios (filas: tiempo, columnas: activos).
pub fn calcular_retornos_diarios(precios: &DMatrix<f64>) -> DMatrix<f64> {
    ...
    for c in 0..cols {
        for r in 1..rows {
            ...
            retornos[(r - 1, c)] = (actual / anterior.max(1e-12)) - 1.0;
        }
    }
    retornos
}

/// Calcula el retorno esperado (media aritmética) por cada activo.
...
pub fn calcular_matriz_covarianza(retornos: &DMatrix<f64>, poblacional: bool) -> DMatrix<f64> {
    ...
    (centrados.transpose() * centrados) / divisor
}

/// Calcula los Betas de los activos respecto al índice de mercado (SPY).
pub fn calcular_betas(retornos: &DMatrix<f64>, serie_mercado: &DVector<f64>, poblacional: bool) -> DVector<f64> {
    ...
    var_mercado /= divisor;
    let var_mercado_safe = var_mercado.max(1e-12);
    ...
    for c in 0..cols {
        ...
        betas[c] = cov_activo_mercado / var_mercado_safe;
    }
...
mod tests {
    ...
    #[test]
    fn test_retorno_esperado() {
        let retornos = dmatrix![
            0.10, -0.10;
            0.10, -0.10
        ];
        let esperados = calcular_retorno_esperado(&retornos);
        assert!((esperados[0] - 0.10).abs() < 1e-6);
        assert!((esperados[1] + 0.10).abs() < 1e-6);
    }
}
